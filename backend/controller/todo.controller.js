...
export const createTodo = async (req, res) => {
  ...
  const todo = new Todo({
    ...
    user: req.body._id, 
   //associate todo with loggedin user
  });
...
export const getTodos = async (req, res) => {
  try {
    const todofetch = await Todo.find({user: req.body._id}); // fetch todos only for loggedin user.
    res.status(201).json({ message: "Todo Fetched Successfully", todofetch });
  } catch (error) {
    console.log(error);
